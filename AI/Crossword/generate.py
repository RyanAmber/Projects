...
class CrosswordCreator():
        ...
        for word in self.domains[x].copy():
            ...
            for word2 in self.domains[y].copy():
                if self.crossword.overlaps[x, y] is not None:
                    ...
                    if word[i] == word2[j] and word!= word2:
            ...
            if not satisfies_constraint:
                domainx.remove(word)
        if domainx != self.domains[x]:
            self.domains[x] = domainx
            return True
        return False
    ...
    def order_domain_values(self, var, assignment):
        ...
        """
        for var2 in self.crossword.variables:
            if var != var2 and self.crossword.overlaps[var, var2] is not None:
                i, j = self.crossword.overlaps[var, var2]
                for word in self.domains[var].copy():
                    count = 0
                    for word2 in self.domains[var2].copy():
                        if word[i] != word2[j]:
                            count += 1
                    yield (word, count)
    ...
    def select_unassigned_variable(self, assignment):
        ...
        for var in self.crossword.variables:
            if var not in assignment:
                ...
                degree = sum(1 for v in self.crossword.variables if v != var and self.crossword.overlaps[var, v] is not None)
                if remaining_values < minTies or (remaining_values == minTies and degree > maxDegree):
                    minTies = remaining_values
                    maxDegree = degree
                    selectedVar = var
        return selectedVar
